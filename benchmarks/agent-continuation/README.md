...
From the repository root, build the libraries and workers with `pnpm run build:bench`, then run `pnpm exec vitest run --config scripts/vitest/bench.config.ts benchmarks/agent-continuation/agent-continuation.bench.ts`. Do not overlap timing runs with builds or other benchmarks.
