...
function parseMode(raw: string | undefined): Mode {
  switch (raw) {
    ...
    default:
      throw new Error(
        `run-gates: expected mode ci-primary | ci-linux-primary | ci-static | ci-lint-contracts-ready | ci-coverage | ci-bench | ci-snapshot | ci-artifacts | ci-consumers | ci-windows-blocking | ci-windows-complete | ci-windows-observational | node-compat | check-all | hygiene, got ${JSON.stringify(raw)}.`,
      )
  }
}
...
function ciWindowsCompleteGates(): Gate[] {
  ...
  return [ciBuildGate(), ...coverage, ...observational]
}
