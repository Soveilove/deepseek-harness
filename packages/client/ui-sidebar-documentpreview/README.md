...
PDF preview bundles the same-version PDF.js legacy runtime and module Worker, including the library's compatibility polyfills for environments without a global `Iterator`. Bundled license notices include core-js; no polyfill is fetched from a CDN.
